...
> **Atualização (§14)**: "single-node" acima é sobre a execução de **um** pipeline (nunca paralelizada entre máquinas). Rodar **múltiplas réplicas** do processo `nexus-server` inteiro (cada uma executando pipelines diferentes, ou a mesma réplica líder do scheduler) já é seguro *storage-wise* com o backend Postgres dos metadados (§14) — o que ainda não existe é o manifest k8s pronto pra isso (Deployment/Service/PVC/HPA), só o pré-requisito de dados.
...
- **Fora de escopo**: manifests k8s reais (Deployment/Service/PVC/HPA) — essa entrega resolve só o pré-requisito de storage/coordenação (ver nota no §6).
