...
* **Windows:** `.msi` real via `cargo-wix` + `packaging/windows/main.wxs`, buildado no CI (`build-windows`, runner self-hosted `windows-connectors-heavy` — mesma máquina do `connectors-heavy.yml`) a cada push/PR pra `main` — **build automatizado, mas ainda não instalado/testado numa máquina Windows real por um humano**. Só a binário do servidor por ora: build dos drivers ADBC (Postgres/SQLite) pra `.dll` ainda não existe (MSVC+vcpkg), então esses 2 conectores não têm driver nativo no Windows ainda — os demais conectores funcionam normalmente. `winget` ainda não configurado.
