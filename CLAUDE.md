...
* **Windows:** `.msi` real via `cargo-wix` + `packaging/windows/main.wxs`, **funcionando** desde que o setup manual da máquina (`vcpkg install openssl:x64-windows-static-md` + `vcpkg integrate install` + `VCPKG_ROOT`, resolvendo o `mysql_cdc`/OpenSSL nativo sem rustls que travava o build) foi feito no self-hosted `windows-connectors-heavy` — `.github/workflows/build-windows-installer.yml` (workflow separado, só `workflow_dispatch`, não faz parte da chain `ci.yml → connectors-heavy.yml → release.yml`) já rodou com sucesso e anexou um `.msi` real (`nexusflow-0.1.3-x86_64.msi`) à release `v0.1.3` no GitHub. Ainda não foi instalado/testado numa máquina Windows real por um humano, e só ship o binário do servidor — build dos drivers ADBC (Postgres/SQLite) pra `.dll` continua sem existir (MSVC+vcpkg separado do OpenSSL acima). O job `build-windows` dentro do `release.yml` (full `connectors-all`, diferente do `.msi` standalone acima) continua **removido dessa chain automática** por ora — religar é decisão separada, não um bloqueio técnico pendente.
