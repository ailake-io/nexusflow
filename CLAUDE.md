...
* **Windows:** `.msi` real via `cargo-wix` + `packaging/windows/main.wxs`, buildado no CI (`build-windows`, runner `windows-latest` hospedado) a cada push/PR pra `main` — **build automatizado, mas ainda não instalado/testado numa máquina Windows real por um humano**. Só a binário do servidor por ora: build dos drivers ADBC (Postgres/SQLite) pra `.dll` ainda não existe (MSVC+vcpkg), então esses 2 conectores não têm driver nativo no Windows ainda — os demais conectores funcionam normalmente. `winget` ainda não configurado.
* **macOS:** `Homebrew` e `.dmg` — specs em `packaging/macos/`, **não validados em máquina real e não wireados no CI de release** (fora de escopo por ora). Faltam scripts de build dos drivers ADBC pra `.dylib`.
* **Docker / K8s:** Imagem Docker multi-arch (amd64 + arm64 via QEMU) publicada em **dois registries** — GHCR e Docker Hub (`ailake/nexusflow`, requer secrets `DOCKERHUB_USERNAME`/`DOCKERHUB_TOKEN` configurados no repo) — a cada push pra `main` (modelo de release contínua a partir da versão em `Cargo.toml`, não mais por tag `git`; ver header de `release.yml`). Container roda como usuário não-root (`nexusflow`, uid 1001) com `HEALTHCHECK` em `/health`. Suporte a GPU via `--gpus all` com base image `nvidia/cuda`, mas aceleração real ainda não validada.
