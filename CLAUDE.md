...
### 4.5. LLMOps (Node LLM, RAG, Avaliação e Versionamento Git)
Mergeado em `develop` desde 2026-09-07 (`518cfa3`, #79) — plano marco a marco em `docs/LLMOPS_IMPLEMENTATION_PLAN.md`, detalhe arquitetural em `ARCHITECTURE.md §17-18`.
...
* **Versionamento git embutido:** todo save de pipeline/prompt também comita num repo git local (`git_history_store.rs`, feature `version-history`, offline/OSS) — mirror opcional pro GitHub via `PUT /settings/git-remote` é a parte paga (`ARCHITECTURE.md §18`).
* **Empacotamento:** node `llm`/custo/cache/prompt/eval, versionamento git local e o próprio `POST /rag/query` são OSS. Pagos: `GET /lineage/generation/{id}` (`llm-lineage-tracking`), a combinação CDC+embedding do RAG reativo (`reactive-rag-cdc`) e o mirror git→GitHub (`git-history-github-sync`) — as únicas 3 capacidades do NexusFlow gateadas por license que **não** são um conector plugável (o código já roda sempre no binário público; ver `ARCHITECTURE.md §17` pro porquê disso mudar como o slug é registrado). Vendidas na Store como 3 produtos separados desde 2026-09-08/09, mesmo checkout Stripe já validado pro Excel (`docs/ENTERPRISE_LICENSING.md`).
...
* **Windows:** `.msi` real via `cargo-wix` + `packaging/windows/main.wxs`, workflow separado (`.github/workflows/build-windows-installer.yml`) em `windows-latest` hospedado — o setup vcpkg/OpenSSL que antes era manual e único na máquina self-hosted virou um passo explícito (`vcpkg install openssl:x64-windows-static-md`) rodando do zero a cada execução, contra o vcpkg que a própria imagem `windows-latest` já traz. Timeout bumpado pra 150min depois que uma execução real mostrou que `cargo build --features connectors-all` sozinho leva 44+ min. Desde 2026-09-08 dispara sozinho (`workflow_run` em `Release`, branch `main`) além do `workflow_dispatch` manual — ver o item "CI/CD encadeado" abaixo. Já instalado e validado numa máquina Windows real (2026-09-06, `.msi` publicado de verdade) — só ship o binário do servidor; build dos drivers ADBC (Postgres/SQLite) pra `.dll` continua sem existir. Manifesto `winget` (`Ailake.NexusFlow` v0.1.6) submetido 2026-09-18, PR pendente de review externo em `microsoft/winget-pkgs`. O job `build-windows` dentro do `release.yml` (full `connectors-all`, diferente do `.msi` standalone acima) continua **removido dessa chain** por ora — religar é decisão separada, não um bloqueio técnico pendente.
* **macOS:** repo ficou público 2026-09-05 (todo runner hospedado é grátis/ilimitado, resolvendo o bloqueio de billing que existia antes) — `release.yml`'s `build` job ganhou leg `macos`/`arm64` no `macos-latest` hospedado nesse mesmo dia (binário OSS-only: o passo que builda o binário enterprise usa `docker build`, indisponível nos runners macOS hospedados). `scripts/build-adbc-*-driver.sh` ganharam suporte a `.dylib`. **Correção 2026-09-09**: doc anterior dizia "não validado" e "sem conectores enterprise" — ambos errados. `build-macos-installer.yml` (workflow separado, mesmo truque `[patch]` de Cargo do Windows em vez de Docker) **rodou de verdade num runner `macos-latest` real em 2026-09-06** (63m54s, `connectors-all` + todo conector enterprise compilando limpo, achou e corrigiu bugs reais no processo — `libpq` keg-only no Homebrew, scripts `duckdb`/`clickhouse` hardcodando extensão Linux) e dispara sozinho desde 2026-09-08 (`workflow_run` em `Release`, mesmo padrão do Windows). Tap dedicado `ailake-io/homebrew-nexusflow` criado 2026-09-17, formula atualizada pra `v0.1.6` (`sha256` real, confirmado batendo com o asset publicado) — `brew install ailake-io/nexusflow/nexusflow` funciona sem passo extra. `packaging/macos/nexusflow.rb` (instalação sem tap) segue mantido em paralelo, mesma versão. O que ainda não aconteceu: um humano rodando `brew install` numa máquina Mac física de verdade.
* **Docker / K8s:** Imagem publicada **só no Docker Hub** (`thiagolange/nexusflow`) desde 2026-09-08 — GHCR foi removido do `release.yml` de vez (job `docker-publish` deletado), substituído por um workflow próprio (`.github/workflows/docker-hub-publish.yml`, dispara via `workflow_run` em `Release` + `workflow_dispatch`), autenticando com `secrets.DOCKERHUB_USERNAME`/`DOCKERHUB_TOKEN` (confirmados existentes no repo). Tags `v{X.Y.Z}` e `latest`, build com `FEATURES=embed-ui,connectors-all`. Container roda como usuário não-root (`nexusflow`, uid 1001) com `HEALTHCHECK` em `/health`. Suporte a GPU via `--build-arg RUNTIME_IMAGE=nvidia/cuda:...` + `--gpus all`, mas aceleração real ainda não validada. **Kubernetes**: manifests em `packaging/kubernetes/` (kustomize), validados num minikube real 2026-09-06/07 — multi-réplica com Postgres compartilhado, health probes, HPA+metrics-server; tag da imagem parametrizada via kustomize's `images:` transformer (não `${VAR}` shell-style, que o `kubectl apply -k` nunca substitui — bug real encontrado e corrigido nessa validação).
* **CI/CD encadeado** (2026-09-08): em `develop`, `ci.yml` e `connectors-heavy.yml` disparam incondicionalmente em todo push (sem filtro de path). Em `main`, a chain é toda automática: `ci.yml` → `connectors-heavy.yml` → (ambos verdes) → `release.yml` → em paralelo, `docker-hub-publish.yml` + `build-windows-installer.yml` + `build-macos-installer.yml` — nenhum desses 3 últimos precisa mais de `workflow_dispatch` manual pra rodar numa release real (o dispatch manual continua disponível pra rebuild avulso).
