...
* **macOS:** repo ficou público 2026-09-05 (todo runner hospedado é grátis/ilimitado, resolvendo o bloqueio de billing que existia antes) — `release.yml`'s `build` job ganhou leg `macos`/`arm64` no `macos-latest` hospedado nesse mesmo dia; `scripts/build-adbc-*-driver.sh` ganharam suporte a `.dylib`. Workflow separado (`build-macos-installer.yml`) também dispara sozinho desde 2026-09-08 (`workflow_run` em `Release`, branch `main`), mesmo padrão do Windows acima. **Ainda não validado numa release real de ponta a ponta por um humano.** Limitação real (não temporária até religar CI, uma decisão de arquitetura): esse binário **não** inclui conectores enterprise — o passo que builda isso usa `docker build`, indisponível nos runners macOS hospedados; Windows contorna isso com um `[patch]` de Cargo em vez de Docker, mesmo truque ainda não replicado pro macOS. Formula Homebrew em `packaging/macos/nexusflow.rb` (checksum placeholder até a primeira release real publicar o tarball).
