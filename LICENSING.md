...
- `nexus-connectors` (subset OSS) — conectores fast-path ADBC (Postgres, SQLite, DuckDB, ClickHouse sink append-only); bridging (REST genérico, MongoDB, Kafka source+sink, Redis Streams, NATS core, RabbitMQ, ODBC, MySQL batch, CSV, webhook, MQTT); data lake (Delta Lake, Iceberg, Parquet, AI Lake); vetoriais (LanceDB, Qdrant, Milvus, pgvector, Pinecone, ChromaDB); e CDC nativo (Postgres WAL, MongoDB Change Streams, MySQL binlog)
