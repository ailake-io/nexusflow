...
5. ~~**Windows (`.msi`) sem CI, nunca buildado**~~ — parcialmente resolvido: `build-windows` job novo em `.github/workflows/release.yml` (`windows-latest` hospedado) roda `cargo build --release` + `cargo wix` contra `packaging/windows/main.wxs` a cada push/PR pra `main`, produz `.msi` real como artifact. **Ainda não instalado/testado numa máquina Windows real por um humano**, e só ship a binário do servidor — build dos drivers ADBC (Postgres/SQLite) pra `.dll` continua sem existir (MSVC+vcpkg), então esses 2 conectores não têm driver nativo no Windows ainda. `winget` continua não configurado. macOS (Homebrew/`.dmg`): specs em `packaging/macos/`, ainda não wireados no CI de release (fora de escopo dessa rodada) e nunca validados em máquina real. Falta também um build script dos drivers ADBC pra macOS (`.dylib`) — os scripts atuais (`scripts/build-adbc-*.sh`) só geram `.so`.
