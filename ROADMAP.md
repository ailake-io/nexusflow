...
- [x] Empacotamento: AppImage/deb/rpm (Linux, todos testados, e desde a rodada de release CI abaixo buildados automaticamente em CI) — `.msi` (Windows) roda em CI (`build-windows-installer.yml`, dispara sozinho após cada release desde 2026-09-08, ver item 5 das Pendências ativas) e já foi instalado numa máquina Windows real (2026-09-06); Homebrew/dmg (macOS) tem workflow próprio também disparando sozinho, mas ainda não validado numa máquina macOS real; winget (Windows) continua sem spec/CI nenhum.
