...
5. ~~**Windows (`.msi`) sem CI, nunca buildado**~~ — parcialmente resolvido: `build-windows` job novo em `.github/workflows/release.yml`, no runner self-hosted `windows-connectors-heavy` (mesma máquina do `connectors-heavy.yml` — trocado do `windows-latest` hospedado depois que este bateu num bloqueio de billing da org, ver item abaixo) roda `cargo build --release` + `cargo wix` contra `packaging/windows/main.wxs` a cada push/PR pra `main`, produz `.msi` real como artifact. **Ainda não instalado/testado numa máquina Windows real por um humano**, e só ship a binário do servidor — build dos drivers ADBC (Postgres/SQLite) pra `.dll` continua sem existir (MSVC+vcpkg), então esses 2 conectores não têm driver nativo no Windows ainda. `winget` continua não configurado. **macOS removido do matrix de release CI por ora** (não é o mesmo caso do Windows: não existe runner self-hosted de macOS pra trocar, e os runners hospedados `macos-13`/`macos-14` do GitHub bateram no mesmo bloqueio de billing — a falha deles cancelava os builds Linux também via fail-fast do matrix). Homebrew/`.dmg`: specs em `packaging/macos/`, nunca validados em máquina real, sem build script dos drivers ADBC pra macOS (`.dylib`) — os scripts atuais (`scripts/build-adbc-*.sh`) só geram `.so`.
...
16. **Billing da org GitHub bloqueado — afeta todo runner hospedado, não só macOS/Windows.** Descoberto ao mergear a PR do item 5: `build (linux, x86_64, ubuntu-latest)` e `build (linux, arm64, ubuntu-24.04-arm)` falharam com o mesmo erro ("recent account payments have failed or your spending limit needs to be increased") que já tinha bloqueado macOS/Windows — ou seja, é bloqueio de conta inteira, não uma cota específica de runner caro. Mitigado parcialmente: `build`'s entrada `linux/x86_64` movida pro self-hosted `[self-hosted, Linux]` (mesma máquina do `ci.yml`), sem dependência de billing. `linux/arm64` continua sem alternativa self-hosted, bloqueado até o billing ser resolvido em `Settings > Billing and plans` — só o usuário (admin da org) resolve isso.
...
- [x] Empacotamento: AppImage/deb/rpm (Linux, todos testados, e desde a rodada de release CI abaixo buildados automaticamente em CI) — `.msi` (Windows) agora também builda em CI (`build-windows` job, ver item 5 das Pendências ativas) mas sem instalação testada numa máquina real; winget (Windows) e Homebrew/dmg (macOS) têm specs em `packaging/` mas não foram validados em máquina real nem wireados em CI
