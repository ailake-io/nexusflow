...
/// Turns a `RecordBatch` into row objects for an outbound JSON body — mirrors
/// every other bridging connector's `batch_to_json_rows` (odbc, mongodb's
/// `RecordBatchBuilder` counterpart), just for `nexus-connector-rest`'s
/// sink instead of a database driver.
