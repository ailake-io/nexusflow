...
pub struct KinesisConnectorConfig {
    ...
    /// Optional custom endpoint — used to target a LocalStack instance
    /// or a VPC endpoint instead of the real AWS Kinesis service.
    #[serde(default)]
    ...
    /// Stop the source after this many consecutive empty poll cycles
    /// across all shards. `0` (default) means never stop — the normal
    /// streaming behavior for production. Set it to a small positive
    /// number (e.g. `5`) in batch/integration tests so the pipeline
    /// terminates once the stream has been drained.
    #[serde(default)]
    pub stop_after_empty_polls: u64,
    ...
    /// Sink-only: column whose stringified value becomes the Kinesis
    /// partition key (controls which shard a record lands on). Unset by
    /// default — the sink then derives a key by hashing the row's JSON
    /// encoding, spreading rows across shards without requiring the
    /// caller to pick a column. Ignored by the source.
    #[serde(default)]
