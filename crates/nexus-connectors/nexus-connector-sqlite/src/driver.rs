...
/// Path to `libadbc_driver_sqlite.so`. No crates.io distribution of the
/// driver exists — build it with `scripts/build-adbc-sqlite-driver.sh` and
/// point this env var at the output. See ARCHITECTURE.md §3.
pub const DRIVER_PATH_ENV: &str = "ADBC_DRIVER_SQLITE_PATH";
...
pub(crate) fn open_connection(uri: &str) -> Result<ManagedConnection, NexusError> {
    let driver_path = env::var(DRIVER_PATH_ENV).map_err(|_| {
        NexusError::Connector(format!(
            "{DRIVER_PATH_ENV} not set — point it at libadbc_driver_sqlite.so \
             (run scripts/build-adbc-sqlite-driver.sh)"
        ))
    })?;
