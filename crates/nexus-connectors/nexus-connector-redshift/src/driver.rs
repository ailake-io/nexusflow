...
/// Path to `libadbc_driver_postgresql.so` — **the same driver and env
/// var name** `nexus-connector-postgres` (public repo) uses, not a
/// Redshift-specific one, since Redshift is wire-compatible with the
/// Postgres protocol and needs no driver changes at all, only a
/// different host/port. Build it with
/// `scripts/build-adbc-postgresql-driver.sh` (copied verbatim from the
/// public repo — see that script's own comment).
