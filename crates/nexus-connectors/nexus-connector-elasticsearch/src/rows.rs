...
/// Turns every column of `batch` except `skip` into a JSON `_source`
/// object per row — same role `nexus-connector-chromadb`'s
/// `batch_to_metadata` (public repo) plays for Chroma metadata, here
/// building the Elasticsearch/OpenSearch document body instead. Kept
...
mod tests {
    ...
    #[test]
    fn batch_to_source_skips_named_columns() {
        ...
        let source = batch_to_source(&batch, &["__opcode"]).unwrap();
        assert_eq!(source.len(), 1);
        let obj = source[0].as_object().unwrap();
