...
use crate::schema::{build_schema, describe_table, VerticaColumn};
...
/// trade-off Oracle/HANA/Teradata accept for the same reason.
pub struct VerticaSource {
    config: VerticaConnectorConfig,
    schema: SchemaRef,
    columns: Vec<VerticaColumn>,
}
...
impl VerticaSource {
    pub async fn connect(config: &VerticaConnectorConfig) -> Result<Self, NexusError> {
        ...
        let columns = retry_with_backoff(&retry, "vertica describe_table", move || {
            let cfg = cfg.clone();
            async move {
                with_timeout(timeout_seconds, "vertica describe_table", async {
                    tokio::task::spawn_blocking(
                        move || -> Result<Vec<VerticaColumn>, NexusError> {
                            let env = Environment::new()
                                .map_err(|e| NexusError::Connector(format!("vertica env: {e}")))?;
                            let conn = env
                                ...
                                .map_err(|e| {
                                    NexusError::Connector(format!("vertica connect: {e}"))
                                })?;
...
fn fetch_all_inner(
    ...
    let mut cursor = conn
        .execute(&sql, (), None)
        .map_err(|e| NexusError::Connector(format!("vertica query failed: {e}")))?
        .ok_or_else(|| NexusError::Connector("vertica SELECT returned no result set".into()))?;
    ...
    let send_batch = |buffer: &mut Vec<Value>| -> Result<(), NexusError> {
        ...
        tx.blocking_send(Ok(batch))
            .map_err(|_| NexusError::Connector("vertica reader: receiver dropped".into()))
    };

    while let Some(mut row) = cursor
        .next_row()
        .map_err(|e| NexusError::Connector(format!("vertica fetch failed: {e}")))?
    {
    ...
    let value = match arrow_type {
        DataType::Int64 => {
            ...
            row.get_data(col, &mut target)
                .map_err(|e| NexusError::Connector(format!("vertica get_data failed: {e}")))?;
        ...
        _ => {
            ...
            let has_value = row
                ...
                .map_err(|e| NexusError::Connector(format!("vertica get_text failed: {e}")))?;
            if has_value {
                Value::String(
...
impl Source for VerticaSource {
    async fn read_batches(
        ...
        let rx = retry_with_backoff(&retry, "vertica source fetch", move || {
            ...
            match tokio::time::timeout(idle_timeout, rx.recv()).await {
                ...
                Err(_) => Some((
                    Err(NexusError::Connector(format!(
                        "vertica cursor stalled for more than {}s",
