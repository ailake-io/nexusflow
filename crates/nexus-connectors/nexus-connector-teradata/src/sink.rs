...
///
/// Upsert is Teradata's `UPDATE ... ELSE INSERT` idiom (see `sql.rs`)
/// — CDC delete opcodes get a real `DELETE`, same as Oracle/HANA.
pub struct TeradataSink {
...
impl TeradataSink {
    pub async fn connect(config: &TeradataConnectorConfig) -> Result<Self, NexusError> {
        ...
        std::thread::Builder::new()
            .name("nexus-teradata-sink".to_string())
            .spawn(move || {
                if let Err(e) = run_worker(config, rx) {
                    tracing::error!("teradata sink worker terminated with error: {e}");
                }
            })
            .map_err(|e| NexusError::Connector(format!("failed to spawn teradata worker: {e}")))?;
...
fn run_worker(
    ...
    let conn: Connection<'_> = retry_sync(&config.retry, "teradata sink connect", || {
    ...
    conn.set_autocommit(false)
        .map_err(|e| NexusError::Connector(format!("teradata set_autocommit(false): {e}")))?;
...
fn apply_batch(
    ...
    match result {
        Ok(()) => conn
            .commit()
            .map_err(|e| NexusError::Connector(format!("teradata commit failed: {e}"))),
        Err(e) => {
            let _ = conn.rollback();
            Err(e)
        }
...
/// Binds params in the exact order `build_upsert_sql` documents: every
/// non-primary-key column (for `SET`), then the primary key (for
/// `UPDATE`'s `WHERE`), then every column in original order (for the
/// `ELSE INSERT`'s `VALUES`) — the primary key cell ends up bound
/// twice (once for `WHERE`, once inside the `INSERT` values), which is
/// correct and expected for this two-statement idiom.
fn upsert(
    config: &TeradataConnectorConfig,
    conn: &Connection<'_>,
    batch: &RecordBatch,
    ...
    let mut stmt = conn
        .preallocate()
        .map_err(|e| NexusError::Connector(format!("teradata preallocate upsert: {e}")))?;
    for row in 0..batch.num_rows() {
        let mut params: Vec<Box<dyn InputParameter>> = Vec::with_capacity(columns.len() * 2);
        for (idx, _) in columns.iter().enumerate() {
            if idx != pk_col {
                params.push(cell_to_param(batch, row, idx)?);
            }
        }
        params.push(cell_to_param(batch, row, pk_col)?);
        ...
        retry_sync(
            ...
            || -> Result<(), NexusError> {
                stmt.execute(&sql, params.as_slice())
                    .map_err(|e| NexusError::Connector(format!("teradata upsert failed: {e}")))?;
                Ok(())
            },
        )?;
    }
...
fn delete(
    ...
    let mut stmt = conn
        .preallocate()
        .map_err(|e| NexusError::Connector(format!("teradata preallocate delete: {e}")))?;
    for row in 0..batch.num_rows() {
        ...
        retry_sync(
            ...
            || -> Result<(), NexusError> {
                stmt.execute(&sql, std::slice::from_ref(&param))
                    .map_err(|e| NexusError::Connector(format!("teradata delete failed: {e}")))?;
                Ok(())
            },
        )?;
    }
...
impl Sink for TeradataSink {
    async fn write_batch(&mut self, batch: RecordBatch) -> Result<(), NexusError> {
        ...
        self.tx
            ...
            .map_err(|_| {
                NexusError::Connector("teradata sink worker has terminated".to_string())
            })?;
        ...
        match tokio::time::timeout(std::time::Duration::from_secs(self.timeout_seconds), rx).await {
            ...
            Ok(Err(e)) => Err(NexusError::Connector(format!(
                "teradata worker response dropped: {e}"
            ))),
            Err(_) => Err(NexusError::Connector(format!(
                "teradata worker did not respond within {}s (driver call still running on its own thread)",
