...
/// mirrors every other bridging connector's `batch_to_json_rows` (odbc,
/// mongodb, rest), covering exactly the four primitives `KafkaDataType`
/// supports (see `config.rs`), so a row produced here round-trips back
/// through `payload.rs::build_schema` on the consumer side.
...
mod tests {
    ...
    #[test]
    fn round_trips_typed_columns_to_json_rows() {
    ...
    fn empty_batch_produces_no_rows() {
        ...
        let batch =
            RecordBatch::try_new(schema, vec![Arc::new(Int64Array::from(Vec::<i64>::new()))])
                .unwrap();
        assert!(batch_to_json_rows(&batch).unwrap().is_empty());
    }
}
