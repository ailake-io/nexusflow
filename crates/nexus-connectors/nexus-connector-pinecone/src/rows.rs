...
/// per row — Pinecone metadata values are limited to string/number/bool
/// (and lists of strings, not produced here), which is exactly what
/// `RecordBatchBuilder`'s 4 supported primitive types map to.
...
/// Reads `column_name` as string vector IDs — Pinecone IDs are always
/// strings, so both `Int64` and `Utf8` primary key columns are supported
/// (unlike Qdrant, which only takes unsigned ints or UUIDs).
