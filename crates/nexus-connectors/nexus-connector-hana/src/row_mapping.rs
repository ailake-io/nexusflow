...
/// schema — same approach the Oracle connector's `row_mapping.rs`
/// (this repo) uses, and for the same reason: both connectors get
/// Arrow types from real catalog introspection (`describe_table`),
/// not a separately-configured type enum.
    ...
    Ok(match column.data_type() {
        ...
        other => {
            return Err(NexusError::Schema(format!(
                "hana sink does not support arrow type {other:?}"
            )))
        }
    })
}
