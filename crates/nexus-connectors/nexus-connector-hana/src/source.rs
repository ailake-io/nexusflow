...
use crate::schema::{build_schema, describe_table, HanaColumn};
...
/// `Send` — same constraint the Oracle connector's `source.rs` (this
/// repo) and `nexus-connector-odbc` (public repo) document. Every ODBC
/// call runs inside `spawn_blocking`, never held across an `.await`.
///
/// `connect()` opens one short-lived connection just to run
/// `describe_table` (needs a live connection to query
/// `SYS.TABLE_COLUMNS`); `read_batches()` opens a second, separate
/// connection for the actual `SELECT` — same two-connections trade-off
/// the Oracle connector accepts for the same reason.
pub struct HanaSource {
    config: HanaConnectorConfig,
    schema: SchemaRef,
    columns: Vec<HanaColumn>,
}
...
impl HanaSource {
    pub async fn connect(config: &HanaConnectorConfig) -> Result<Self, NexusError> {
        ...
        let columns = retry_with_backoff(&retry, "hana describe_table", move || {
            let cfg = cfg.clone();
            async move {
                with_timeout(timeout_seconds, "hana describe_table", async {
                    tokio::task::spawn_blocking(move || -> Result<Vec<HanaColumn>, NexusError> {
                        let env = Environment::new()
                            .map_err(|e| NexusError::Connector(format!("hana env: {e}")))?;
                        let conn = env
...
fn fetch_all_inner(
    ...
    let mut cursor = conn
        .execute(&sql, (), None)
        .map_err(|e| NexusError::Connector(format!("hana query failed: {e}")))?
        .ok_or_else(|| NexusError::Connector("hana SELECT returned no result set".into()))?;
    ...
    let send_batch = |buffer: &mut Vec<Value>| -> Result<(), NexusError> {
        ...
        tx.blocking_send(Ok(batch))
            .map_err(|_| NexusError::Connector("hana reader: receiver dropped".into()))
    };

    while let Some(mut row) = cursor
        .next_row()
        .map_err(|e| NexusError::Connector(format!("hana fetch failed: {e}")))?
    {
    ...
    let value = match arrow_type {
        DataType::Int64 => {
            ...
            row.get_data(col, &mut target)
                .map_err(|e| NexusError::Connector(format!("hana get_data failed: {e}")))?;
        ...
        _ => {
            ...
            let has_value = row
                ...
                .map_err(|e| NexusError::Connector(format!("hana get_text failed: {e}")))?;
            if has_value {
                Value::String(
...
impl Source for HanaSource {
    async fn read_batches(
        ...
        let rx = retry_with_backoff(&retry, "hana source fetch", move || {
            ...
            match tokio::time::timeout(idle_timeout, rx.recv()).await {
                ...
                Err(_) => Some((
                    Err(NexusError::Connector(format!(
                        "hana cursor stalled for more than {}s",
