...
impl DbtTestResultStore {
    ...
    /// Removes every row recorded for `pipeline_id` in `dbt_test_results`. Called when
    ...
    pub async fn delete_for_pipeline(&self, pipeline_id: &str) -> anyhow::Result<()> {
        let sql = self.q("DELETE FROM dbt_test_results WHERE pipeline_id = ?");
        match &self.pool {
            MetadataPool::Sqlite(p) => {
...
mod tests {
    ...
    async fn delete_for_pipeline_removes_only_that_pipelines_results() {
        let store = DbtTestResultStore::connect("sqlite::memory:")
            ...
            .unwrap();
        store
            .record_all("pipe-1", 1, &[outcome("test.a", "pass")])
            .await
            .unwrap();
        store
            .record_all("pipe-2", 1, &[outcome("test.a", "pass")])
            .await
            .unwrap();
