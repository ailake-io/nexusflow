...
- **`hugging face hub error: I/O error Permission denied (os error 13)` em qualquer pipeline com node de embedding** (Docker): a imagem criava o usuário `nexusflow` com `useradd -r`, que **não cria home directory** — o runtime do embedding baixa o modelo ONNX para `$HOME/.cache` (nexus-ai via hf-hub), e em container recém-criado o usuário não consegue criar `/home/nexusflow` (EACCES, `/home` é root-owned). Corrigido no Dockerfile (commit `e9001d0`: `mkdir -p /home/nexusflow && chown 1001:1001 /home/nexusflow`). Pra containers já deployados sem rebuild da imagem: `docker exec -u 0 <container> sh -c 'mkdir -p /home/nexusflow && chown 1001:1001 /home/nexusflow'`. Instalação bare-metal/deb/rpm não afetada (o usuário ali é criado com home pelo sistema). Sintoma fácil de mascarar: em container que já rodou antes, o diretório pode ter sido criado na camada writable de uma instância anterior — funciona até o container ser recriado, aí volta a quebrar "do nada".
